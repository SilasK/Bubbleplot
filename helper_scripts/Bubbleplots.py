...
from .plotting import saveplot,count_sample_for_legend
...
## make grid
    ...
    for group_name in order[:-1]:
        ...
        data_group = Grouped_data.get_group(group_name).T
...
def BarPlot(data,colorM='Paired',ax=None,headers='show',value_max=None,x_ticklabels_rotation=90,**kws):
    """sample ids X feature ids"""
    if ax is None:
        ax=plt.subplot(111)
    ...
    data.plot(kind='bar', stacked=True,colormap=colorM, ax=ax,**kws)
